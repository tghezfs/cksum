...
mod tests {
    ...
    // Helper to clean code: creates file, runs function, returns content
    fn run_process(content: &[u8], algo: Algo) -> String {
        let mut input_file = NamedTempFile::new().expect("Failed to create temp input");
        ...
        process_file(path, "test_file.txt", &mut buffer, &algo, &mut output_file)
    ...
    fn test_md5_empty_string() {
        ...
        // MD5 of empty string is known: d41d8cd98f00b204e9800998ecf8427e
    ...
    fn test_sha256_known_string() {
        ...
        // SHA256 of "testing": cf80cd8aed482d5d1527d7dc72fceff84e6326592848447d2dc0b0e87dfc9a90
    ...
    fn test_blake3_known_string() {
        ...
        // Verify format and length only (actual hash can be computed if needed)
    ...
    fn test_large_content() {
        // Test with content larger than buffer (1024 bytes)
        ...
        // Hash length should be 64 hex chars
        let parts: Vec<&str> = res.split_whitespace().collect();
        assert_eq!(parts[1].len(), 64);
    }
    ...
    fn test_small_buffer() {
        ...
        let mut output_file = NamedTempFile::new().expect("Failed to create temp output");
        // Use very small buffer to force multiple reads
        let mut buffer = vec![0u8; 4];

        process_file(path, "test.txt", &mut buffer, &Algo::Md5, &mut output_file)
            .expect("process_file failed");
        ...
        // MD5 of "test content" should be consistent regardless of buffer size
    ...
    fn test_output_format() {
        ...
        // Verify exact format: "algo hash filename"
